...
class BKG_dataholder():
    ...
    def __init__(self,fill_path:str, fill_number:int,
        ...
        self.fill_data_gathered = False
        self.beam_data_gathered = False
        self.vacuum_data_gathered = False
        self.collimator_data_gathered = False
        ...
        if self.beam_path is not None:
            self.beam_variables = {
                ...
                #"beam1_intensity":"LHC.BCTDC.A6R4.B1:BEAM_INTENSITY",
                #"beam2_intensity":"LHC.BCTDC.A6R4.B2:BEAM_INTENSITY",
                "TCTPH_B1":"BLMTI.04L5.B1I10_TCTPH.4L5.B1:LOSS_RS09",
                "TCTPH_B2":"BLMTI.04R5.B2I10_TCTPH.4R5.B2:LOSS_RS09",
                "TCL4_B2":"BLMTI.04L5.B2E10_TCL.4L5.B2:LOSS_RS09",
                "TCL4_B1":"BLMTI.04R5.B1E10_TCL.4R5.B1:LOSS_RS09",
                "TCTPV_B1":"BLMTI.04L5.B1I10_TCTPV.4L5.B1:LOSS_RS09",
                "TCTPV_B2":"BLMTI.04R5.B2I10_TCTPV.4R5.B2:LOSS_RS09",
                "TCL5_B1":"BLMTI.05R5.B1E10_TCL.5R5.B1:LOSS_RS09",
                "TCL5_B2":"BLMTI.05L5.B2E10_TCL.5L5.B2:LOSS_RS09",
                "TCL6_B2":"BLMTI.06L5.B2E10_TCL.6L5.B2:LOSS_RS09",
                "TCL6_B1":"BLMTI.06R5.B1E10_TCL.6R5.B1:LOSS_RS09"
                }
    ...
    def get_vacuum_data(self):
        ...
        self.vacuum_data_gathered = True
    ...
    def get_collimator_data(self):
        ...
        self.collimator_data_gathered = True
    ...
    def get_beam_data(self):
        ...
        self.beam1_intensity = pd.DataFrame()
        self.beam2_intensity = pd.DataFrame()

        self._fill_start = beam_data_dict["start"]
        self._fill_end = beam_data_dict["stop"]
        ...
        #times,values = beam_data_dict[self.beam_variables["beam1_intensity"]]
        #self.beam1_intensity["timestamp"] = times.copy()
        #self.beam1_intensity["time"] = pd.to_datetime(times,unit = "s")
        #self.beam1_intensity["value"] = values.copy()
        
        #times,values = beam_data_dict[self.beam_variables["beam2_intensity"]]
        #self.beam2_intensity["timestamp"] = times.copy()
        #self.beam2_intensity["time"] = pd.to_datetime(times,unit = "s")
        #self.beam2_intensity["value"] = values.copy()

        self.loss_maps = {}
        for k,v in losses.items():
            self.loss_maps.update({k: [
                np.array(beam_data_dict[v][0]),
                np.array(beam_data_dict[v][1])
            ]})




        self.beam_data_gathered = True

        
    @property
    def fill_end_dt(self):
        if not hasattr(self, "_fill_end"):
            beam_data_dict = dict()
            with open(self.beam_path + "/beam_data_" + str(self.fill_number) + ".json",
                    "r") as f:
                beam_data_dict = json.load(f)
        ...
        return datetime.datetime.utcfromtimestamp(self._fill_end)
    ...
    def fill_end(self):
        ...
        return self._fill_end


    @property
    def fill_start(self):
        if not hasattr(self, "_fill_start"):
            beam_data_dict = dict()
            with open(self.beam_path + "/beam_data_" + str(self.fill_number) + ".json",
                    "r") as f:
                beam_data_dict = json.load(f)
    ...
    def fill_start_dt(self):
        ...
        return datetime.datetime.utcfromtimestamp(self._fill_start)
